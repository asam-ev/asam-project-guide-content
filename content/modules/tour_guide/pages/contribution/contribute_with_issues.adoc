...
A project may also have project-specific labels in addition to the default ones, specified directly in the project group.

=== Introduce project-specific labels
A group may introduce project-specific labels on top of the official ASAM ones that provide further filter options.
If custom labels are introduced, the group must document these labels by providing a exhaustive label description with the label (i.e. in the GitLab/GitHub label).
Additionally, the project may add the current label definitions in their project's README in their repository.

The documentation shall follow the project's general documentation rules and language.
...
== Reopen issues
Issues shall not be reopened!
If an issues has been closed by accident or has become relevant for other reasons again, a related issue shall be created instead.
