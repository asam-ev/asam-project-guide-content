...
Each state in the workflow is described in more detail in the sections below.
...
--
[horizontal]
What:: A new issue that needs to be classified to a type.
Who:: <<Workflow Manager>>
State Label:: {isStateNew-label}
Actions::
...
--
[horizontal]
What:: An issue that has a type and that needs to be accepted by the group.
Who:: Relevant lead (project lead or subgroup lead)
State Label:: {isStateClassified-label}
Actions::
...
--
[horizontal]
What:: An issue that has been accepted by the group, which needs to be assigned to a responsible, given a due date and needs a clear specification of requirements to be fulfilled.
Who:: Relevant lead (Project Lead or Subgroup Lead)
State Label:: {isStateAccepted-label}
Actions::
...
--
[horizontal]
What:: An issue that is assigned a responsible, has a due date and clear requirements, which now needs to be implemented in a solution.
Who:: Solution developer (assignee)
State Label:: {isStateInProgress-label}
Actions::
...
* Monitor and discuss developments within solution team
** On solution completion, set a due date for review (should not exceed 2 weeks)
** Inform project lead and/or CCB of issue review 
Transitions:: 
...
--
[horizontal]
What:: An issue that has a solution in an associated merge request or document, which is ready to be reviewed by the group.
Who:: The project lead shall be the process owner and responsible for initiating and ending the review. They may delegate these process to a CCB if present, but this shall be clearly defined within the project.
State Label:: {isStateImplemented-label}
Actions::
* Perform issue review
** Issue on the next agenda for the next project or CCB meeting
** Project group or CCB reviews solution and documents their feedback:
...
--
[horizontal]
What:: An issue that has a been accepted by the group, has an assigned integrator, which is ready to be integrated into the standard or project.
Who:: <<Integrator>>
State Label:: {isStateReviewed-label}
Actions::
...
--
[horizontal]
What:: An issue that has been integrated and has an integration reviewer, which is ready to be checked and all open comments are resolved.
Who:: <<Integration Reviewer>>
State Label:: {isStateIntegrated-label}
Actions::
...
--
[horizontal]
What:: An issue that has been integrated and is assigned to the workflow manager, which is ready to be merged.
Who:: <<Workflow Manager>>
State Label:: {isStateReadyForMerge-label}
Actions::
**Note that this is only applicable to projects with a docs-as-code workflow**
* Merge the assigned Merge/Pull Request
** If any major merge conflicts arise, document conflict, reassign it to the <<Integration Reviewer>>, and change back to the previous state
...
They may be an individual or a group, for example a CCB. 
