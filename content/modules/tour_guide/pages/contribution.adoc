...
** If the issue is a duplicate, assign {resolutionTypeDuplicate-label} and close the issue
** Otherwise, assign the correct type (one of: {isTypeFeature-label}, {isTypeBug-label}, {isTypeDocumentation-label})
DoD::
...
* "Draft:" flag removed from related Merge/Pull request
DoD::
* Solution reviewed 
...
* Add feedback as comments on Merge/Pull Request
* Add verdict as comment in issue
** If accepted, assign issue to the <<Workflow Manager>>

DoR::
* <<Integraton Reviewer>> assigned
DoD::
* Solution reviewed
* Feedback provided in Merge/Pull Request, if relevant
* Verdict documented in issue
Transitions:: 
...
DoD::
* Branch merged
* In case of merge conflicts: conflict documented in Merge/Pull Request
Transitions:: 
...
It is important to note that the due date of the solution does not account for integration!
