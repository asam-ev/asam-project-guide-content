...
However, in order for a consistent workflow and optimal use of the automation steps set up by __{asam}__, users must be familiar with the xref:compendium:Guidelines/trunk_workflow.adoc[] and <<Writing AsciiDoc Files,AsciiDoc>>.
...
Editing AsciiDoc files in a project shall be done in development or feature branches, as described in xref:compendium:Guidelines/trunk_workflow.adoc[].
