...
The approach also relies on xref:compendium:Guidelines/repo-merge-pull_requests.adoc[Merge/Pull Request] which provide better tools and options for reviews than a direct push to the trunk would.
...
You may also want to check out ASAM's xref:compendium:Guidelines/repo-issue_and_mr_labels.adoc#_status_labels[**list of status labels**].
...
.. If changes were developed in xref:compendium:How-Tos/how-to-mark_parts_as_draft.adoc[draft sections], remove the encapsulating draft feature flag from the content (make sure that all other content remains within the draft flag) and commit the change.
.. If there is no xref:compendium:Guidelines/repo-merge-pull_requests.adoc[Merge/Pull Request] for this branch yet, create one
...
* Create branches xref:compendium:How-Tos/how-to-create_branch_from_issue.adoc[directly from the issue] to ensure the branch is linked to the issue.
...
* Mark passages which are not ready for viewing in the final output as draft (see xref:compendium:How-Tos/how-to-mark_parts_as_draft.adoc[])
--
Review::
+
--
...
. The Service Provider refines the content in the xref:compendium:Guidelines/repo-merge-pull_requests.adoc[Merge/Pull Request]
