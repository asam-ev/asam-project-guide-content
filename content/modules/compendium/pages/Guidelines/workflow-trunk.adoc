...
Who:: Assignee
Steps::
. Assign the label {requires:Clarification} to indicate this issue is being looked at and discussed.
...
.. Assign the label {isStatus:Duplicate} and assign the {role-wg-lead}
.. The WG Lead reviews the issue and either closes it (if it is a duplicate) or changes the status back to {requires:Clarification} and the assignee back to the original issue assignee
Tips::
...
. Change the issue status label to {requires:ConceptualWork} to indicate that a proposal is being implemented for this issue
...
.. Change the issue status label to {requires:CCBReview}
...
Tips::
* If at any point during the implementation the group decides the issue has become irrelevant or cannot be implemented, the status label is changed to {isStatus:WontBeFixed} and the issue is closed (as well as any related branches and Merge/Pull Requests)
...
Steps::
. The Service Provider changes the status label to {requires:DocumentationWork}
...
.. Set the status label of the Merge/Pull Request to {requires:CCBReview}
...
. If the review is passed but the CCB may not merge, the CCB changes the status label to {requires:Merge}
Tips::
