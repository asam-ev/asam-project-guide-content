...
|{isStatus:NotInProgress}
|This label indicates that the work on the issue has already begun and is ongoing.

|{isStatus:ReadyForServiceProvider}
|This issue is ready for the service provider.

|{isStatus:WontBeFixed}
...
.Suggested workgroup label schema
[source]
----
'isWorkgroup':'Workgroup'
----
...
Example::
An issue label for the group Domain Model (DomainModel) would be 'isWorkgroup:DomainModel'
