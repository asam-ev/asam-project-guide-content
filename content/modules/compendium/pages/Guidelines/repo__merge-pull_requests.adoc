= [Repository] Merge/Pull Requests
...
== Introduction
A Merge or Pull Requests (MRs) are the way you merge content changes from a development or feature branch back into the xref:compendium:Guidelines/workflow__trunk.adoc#_the_trunk[trunk] or another branch.
...
- Commit messages to a protected or master branch must adhere to the xref:compendium:Guidelines/repo__commit_conventions.adoc[]
...
See xref:compendium:Guidelines/repo__merge_conflicts.adoc[].


== Merge/Pull Request Review
Merge/Pull Requests are reviewed by the project team in sequential steps.
See the xref:compendium:Guidelines/workflow__trunk.adoc#_review[Trunk Workflow] page for more information on these reviews
