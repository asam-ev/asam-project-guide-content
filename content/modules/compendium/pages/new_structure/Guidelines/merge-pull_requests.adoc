...
== Introduction
A Merge or Pull Requests (MRs) are the way you merge content changes from a development or feature branch back into the xref:compendium:new_structure/Guidelines/trunk_workflow.adoc#_the_trunk[trunk] or another branch.
...
- Commit messages to a protected or master branch must adhere to the xref:compendium:new_structure/Guidelines/commit_guidelines.adoc[]
...
See xref:compendium:temp_issues/merge_conflicts.adoc[].


== Merge/Pull Request Review
Merge/Pull Requests are reviewed by the project team in sequential steps.
See the xref:compendium:new_structure/Guidelines/trunk_workflow.adoc#_review[Trunk Workflow] page for more information on these reviews
