...
Cross references use the `full` style which contains of the fixed text "Section", the section number and the title of the target.
The syntax for rendering links is implemented in the ASAM Style Resources repository in the file _preamble.adoc_.

*Exceptions*

Use the `basic` style for cross references if you cite the titles of figures or tables.
