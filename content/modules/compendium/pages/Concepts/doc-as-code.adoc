...
When changes need to be made, they are added through branches created from this trunk.
Each child branch targets a single aspect of development.
Once the development in a child branch has advanced enough, its changes are merged back into the trunk.
This is done through a so-called Merge Request (MR), sometimes also called a Pull Request (PR).
...
People from different companies join in a project to develop something together. +
