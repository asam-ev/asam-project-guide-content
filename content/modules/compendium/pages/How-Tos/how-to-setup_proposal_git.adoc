= How-To: Setup Project Proposal Repository
...
h|TL;DR
a| * If completely new, create new subgroup with the proposed project name in https://code.asam.net/simulation[**Simulation**^] and add members
* If not new, update the assigned members
* Fork the https://code.asam.net/common/templates/proposal-template[**proposal template repository**^] into this proposal subgroup as "<project-name> Proposal"
* Inform members
|===
//end::roles[]
...
. In case of a new standard, https://docs.gitlab.com/ee/user/group/subgroups/#creating-a-subgroup[create a new subgroup^] in the group https://code.asam.net/simulation[**Simulation**^] with the target name of the standard (e.g. "OpenDRIVE").
+
NOTE: If this is an evolution of an existing standard, use the existing subgroup instead
+
.. xref:compendium:How-Tos/how-to-forking_repo.adoc[Fork] the latest version of the https://code.asam.net/common/templates/proposal-template[**proposal template repository**^].
...
. Add all current proposal members to the subgroup (the one directly in "Simulation").
