...
The group also defines other roles, such as the members of the **Change Control Board** (CCB) and the **Subgroup Leads**.
...
* elected or selected subgroup leads that are responsible for certain focus points of the project's topics,
...
Here, the Project Lead defines the **goals for the cycle** together with the members of the CCB and the Subgroup Leads. +
...
Each Subgroup may decide to have one or more internal meetings per cycle.
ASAM recommends to have at least one group meeting every second week. +
If so, these are considered **official meetings** and hosted by the respective Subgroup Lead.
Make sure to follow the xref:contributing:meeting_guidelines.adoc[ASAM meeting guidelines].
...
Each Subgroup presents its results to its Subgroup Lead.
Each Subgroup Lead then presents their group's status to the other Subgroup Leads, the CCB, and the Project Lead.
...
Each Subgroup Review meeting is hosted by the respective Subgroup Lead.
...
Any changes to an underlying data model must be presented at the next <<Project-level meeting,project-level meeting>>.
This is to ensure that work in other subgroups based on that model takes these changes into account as soon as possible.

It is up to a subgroup and its corresponding Lead to determine whether a change in a subgroup should be escalated for approval to a project level decision.
The Project Lead may escalate subgroup decisions to a project level vote.
A group defined by the project that is responsible for change management, such as the CCB, may escalate subgroup changes and decisions to a project level vote as well.
