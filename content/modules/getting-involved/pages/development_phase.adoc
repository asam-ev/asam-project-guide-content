...
This is to ensure that work in other sub-groups based on that model takes these changes into account as soon as possible.

It is up to a sub-group and its corresponding Lead to determine whether a change in a sub-group should be escalated for approval to a project level decision.
The Project Lead may escalate sub-group decisions to a project level vote.
A group defined by the project that is responsible for change management, such as the CCB, may escalate sub-group changes and decisions to a project level vote as well.
