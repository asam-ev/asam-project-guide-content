= Setting Up Your Repository Account
:description: How to setup your ASAM account.
:keywords: task,fragment,setup,gitlab, github,account

include::partial$_attributes.adoc[]
include::{abb}[]

{remote-icon-page}

== Introduction
Before you can get started, you need to create and set up an account for the project's online repository.
Depending on the project, this may either be GitLab or GitHub.

Select and follow the respective instructions in the tab view below.

include::partial$icon_reminder.adoc[tag=online-access]

[tabs]
====
GitLab::
+
--
[cols="^1,8", frame=none, grid=none]
|===
|{gitlab-icon-norole} +
{gitlab-url}[ASAM GitLab^]
a|include::compendium:page$tools/gitlab/how-to__gitlab-setup.adoc[tag=body]
|===
--
GitHub::
+
--
[cols="^1,8", frame=none, grid=none]
|===
|{github-icon-norole} +
{github-url}[ASAM e.V. on GitHub^]
a|include::compendium:page$tools/github/how-to__github-setup.adoc[tag=body]
|===
--
====
